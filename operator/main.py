...
def create_training_job():
    ...
    try:
        ...
        jobs = batch_v1_api.list_namespaced_job(namespace=os.getenv("NAMESPACE"))

        # Filter jobs based on the name
        filtered_jobs = [job for job in jobs.items if job.metadata.name.startswith("training-" + auth_header)]

        for job_name in filtered_jobs:
            job = batch_v1_api.read_namespaced_job(name=job_name, namespace=os.getenv("NAMESPACE"))
            if job.status.active:
                logging.info(f"Training job already exists and is in a running state for {auth_header}")
            ...
            #    batch_v1_api.delete_namespaced_job(name="training-"+auth_header, namespace=os.getenv("NAMESPACE"))
    ...
    job = client.V1Job(
        ...
        kind="Job",
        metadata=client.V1ObjectMeta(name="training-"+auth_header+"-"+random_uuid),
    ...
    try:
        ...
        batch_v1_api.create_namespaced_job(namespace=os.getenv("NAMESPACE"), body=job)
    ...
    try:
        ...
        for job in filtered_jobs:
            if job.status.active:
                ...
                status = "Unknown"
            job_states[job.metadata.name.replace("training-"+auth_header+"-","")] = status
...
def get_training_job(id):
    ...
    try:
        ...
        job = batch_v1_api.read_namespaced_job(name="training-"+auth_header+"-"+id, namespace=os.getenv("NAMESPACE"))
...
def create_serving_deployment():
    ...
    try:
        deployment = apps_v1_api.read_namespaced_deployment(name="serving-"+auth_header, namespace=os.getenv("NAMESPACE"))
        if deployment:
    ...
    deployment = client.V1Deployment(
        api_version="apps/v1",
        kind="Deployment",
        metadata=client.V1ObjectMeta(name="serving-"+auth_header),
    ...
    try:
        ...
        apps_v1_api.create_namespaced_deployment(namespace=os.getenv("NAMESPACE"), body=deployment)
...
def get_serving_deployment():
    ...
    try:
        ...
        deployment = apps_v1_api.read_namespaced_deployment(name="serving-"+auth_header, namespace=os.getenv("NAMESPACE"))
        logging.info(f"Serving deployment status for {auth_header}: {deployment.status}")
        return jsonify({'status': deployment.status}), 200
    except ApiException as e:
        logging.error(f"ApiException occurred: {str(e)}")
        if e.status == 404:
            return jsonify({'error': 'Deployment not found'}), 404
...
def delete_serving_deployment():
    ...
    try:
        ...
        apps_v1_api.delete_namespaced_deployment(name="serving-"+auth_header, namespace=os.getenv("NAMESPACE"))
